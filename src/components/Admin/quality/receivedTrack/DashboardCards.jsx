...
const DELIVERED_GRADES = ["c2", "c1", "s86", "s87", "s88", "A2", "A3", "B1", "B2", "N2"];
const HIGH_GRADES = ["A0", "A1", "N1", "H1"];
    ...
    const renderGradeBreakdown = (gradesObj, total, gradesList) => (
        <div>
            {gradesList.map((grade) => {
                ...
                return (
                    <div key={grade} className="mb-3 p-2 rounded" style={{ background: '#f6fdf7', border: '1px solid #e6f3ec' }}>
                        <div className="d-flex justify-content-between align-items-center mb-1">
                            <span>
                                <Badge
                                    style={{ backgroundColor: processingTheme.green, color: "#fff", minWidth: 40, fontSize: 15, padding: '6px 12px' }}
                                    className="me-2"
                                >
                                    {grade}
                                </Badge>
                                <span style={{ fontWeight: 500, fontSize: 15 }}>
                                    {value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kg
                                </span>
                            </span>
                            <span className="text-muted" style={{ fontSize: 14 }}>{percent}%</span>
                        </div>
                        <ProgressBar
                            now={percent}
                            style={{ height: "8px", backgroundColor: processingTheme.tableHover, borderRadius: 4 }}
                            variant="success"
                        />
                    </div>
                );
                        ...
                        <Col md={3} key={profile} className="mb-3">
                            <Card className="h-100 border-0" style={{ backgroundColor: processingTheme.tableHover }}>
                                <Card.Body>
                                    ...
                                    <h4>{Number(value).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kg</h4>
                                    <div className="progress" style={{ height: "8px" }}>
                                        <div
                                            className="progress-bar"
                                            role="progressbar"
                                            style={{
                                                width: `${totalKgs ? ((value / totalKgs) * 100).toFixed(1) : 0}%`,
                                    ...
                                    <small className="text-muted mt-2 d-block">
                                        {totalKgs ? ((value / totalKgs) * 100).toFixed(1) : 0}% of total
                                    </small>
                                </Card.Body>
                            </Card>
                        </Col>
    ...
    return (
        <>
            ...
            <Row className="mb-4">
                <Col md={3}>
                    <Card className="h-100 shadow-sm" style={{ borderTop: `4px solid ${processingTheme.primary}` }}>
                        <Card.Body>
                            ...
                            <h3 style={{ color: processingTheme.primary }}>{summary.total?.toLocaleString()}</h3>
                        ...
                        <Card.Body>
                            <div className="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <h3 style={{ color: processingTheme.accent }}>{summary.grandTotalTransportedSum?.toLocaleString()} kg</h3>
                            ...
                            {showTransportGrades && (
                                <div className="mt-3">
                                    {renderGradeBreakdown(summary.grandTotalTransported, summary.grandTotalTransportedSum, TRANSPORT_GRADES)}
                                </div>
                            )}
                        </Card.Body>
                    </Card>
                ...
                <Col md={3}>
                    <Card className="h-100 shadow-sm" style={{ borderTop: `4px solid ${processingTheme.gray}` }}>
                        <Card.Body>
                            <div className="d-flex justify-content-between align-items-center">
                                <div>
                                    <h6 className="text-muted mb-1">Total Delivered (Sum)</h6>
                                    <h3 style={{ color: processingTheme.gray }}>{summary.grandTotalDeliveredSum?.toLocaleString()} kg</h3>
                            ...
                            {showDeliveredGrades && (
                                <div className="mt-3">
                                    {renderGradeBreakdown(summary.grandTotalDelivered, summary.grandTotalDeliveredSum, DELIVERED_GRADES)}
                                </div>
                            )}
                        </Card.Body>
                    </Card>
                ...
                <Col md={3}>
                    <Card className="h-100 shadow-sm" style={{ borderTop: `4px solid ${processingTheme.green}` }}>
                        <Card.Body>
                            ...
                            <h3 style={{ color: processingTheme.green }}>
                                {summary.grandTotalTransportedSum
                                    ? ((summary.grandTotalDeliveredSum / summary.grandTotalTransportedSum) * 100).toFixed(1)
                                    : 0}
                                %
                            </h3>
                            <p className="mb-0">{summary.grandTotalDeliveredSum?.toLocaleString()} / {summary.grandTotalTransportedSum?.toLocaleString()} kg</p>
                        </Card.Body>
                    </Card>
                </Col>
            </Row>
