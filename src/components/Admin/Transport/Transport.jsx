...
const Transport = () => {
  ...
  // New function to calculate summary data based on filtered records
  const calculateFilteredSummaryData = () => {
    const filteredGroups = getFilteredGroups();
    const filteredRecords = filteredGroups.flatMap(
    ...
    // Update summary data with filtered values
    setSummaryData({
      totalKgs,
      totalBags,
      highGradeKgs,
  ...
  return (
    <div className="container-fluid py-4">
      ...
      <br></br>
      {/* Dashboard Summary Cards - Now Dynamic */}
      <Row className="mb-4">
        <Col md={3}>
          <Card
          ...
          >
            <Card.Body>
              ...
              <h3 style={{ color: processingTheme.primary }}>
                {summaryData.totalKgs.toLocaleString()} kg
              </h3>
          ...
          >
            <Card.Body>
              ...
              <h3 style={{ color: processingTheme.accent }}>
                {summaryData.highGradeKgs.toLocaleString()} kg
              </h3>
              <p className="mb-0">
                {summaryData.totalKgs > 0
                  ? (
          ...
          >
            <Card.Body>
              ...
              <h3 style={{ color: "#6c757d" }}>
                {summaryData.lowGradeKgs.toLocaleString()} kg
              </h3>
              <p className="mb-0">
                {summaryData.totalKgs > 0
                  ? (
        ...
        <Col md={3}>
          ...
          >
            <Card.Body>
              ...
              <h3 style={{ color: "#28a745" }}>{getFilteredGroups().length}</h3>
              <p className="mb-0">
                {
                  getFilteredGroups().flatMap(([_, group]) => group.records)
                    .length
                }{" "}
                total batches
              </p>
      ...
      {/* Cup Profile Summary Cards - Now Dynamic */}
              ...
              <Col md={3} key={profile} className="mb-3">
                ...
                >
                  <Card.Body>
                    ...
                    <h4>{data.kgs.toLocaleString()} kg</h4>
                    <div className="progress" style={{ height: "8px" }}>
                      <div
                        ...
                        style={{
                          width: `${
                            summaryData.totalKgs > 0
                              ? (data.kgs / summaryData.totalKgs) * 100
                              : 0
                          }%`,
                    ...
                    <small className="text-muted mt-2 d-block">
                      {summaryData.totalKgs > 0
                        ? ((data.kgs / summaryData.totalKgs) * 100).toFixed(1)
                        : 0}
                      % of total
                    </small>
                  </Card.Body>
      ...
      {/* Additional Insights Cards - Now Dynamic */}
      <Row className="mb-4">
                  ...
                  <tbody>
                    {Object.entries(summaryData.washingStationStats).map(
                      ([station, stats]) => (
                        <tr key={station}>
                          <td>{station}</td>
                          <td>{stats.totalKgs.toLocaleString()} kg</td>
                          <td>
                    ...
                    {Object.entries(summaryData.gradeDistribution)
                      ...
                      .map(([grade, kg]) => (
                        <tr key={grade}>
                          ...
                          <td>{kg.toLocaleString()} kg</td>
                          <td>
                            <div className="d-flex align-items-center">
                              <div className="me-2">
                                {summaryData.totalKgs > 0
                                  ? ((kg / summaryData.totalKgs) * 100).toFixed(
                                      1
                                    )
                                  : 0}
                                %
