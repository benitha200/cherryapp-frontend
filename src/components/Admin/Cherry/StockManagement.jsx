...
const StockManagement = () => {
    ...
    const pieColors = ['#008080', '#4FB3B3', '#79CDCD', '#A3E0E0', '#D95032', '#E67E22', '#F39C12', '#FFC300', '#DAF7A6', '#9B59B6'];
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const processingResponse = await fetch(`${API_URL}/processing?limit=100000`);
                ...
                const uniqueCWS = ['All', ...new Set(processingData.map(item => item.cws?.name || 'Unknown'))];
                setCwsList(uniqueCWS);

                // Prepare CWS grade contribution data
                const contributionData = prepareCwsGradeContribution(stockAnalysis.gradeByCWS);
                setCwsGradeContribution(contributionData);

                // Prepare CWS grade distribution data (for horizontal stacked bar chart)
                const distributionData = prepareCwsGradeDistribution(stockAnalysis.gradeByCWS);
    ...
    const prepareDetailedTableData = (gradeByCWS) => {
        return Object.keys(gradeByCWS).map(cws => {
            const cwsData = { cws };
            let total = 0;
    ...
    const prepareCwsGradeDistribution = (gradeByCWS) => {
        ...
        const sortedCWS = cwsList.sort((a, b) => {
            const totalA = Object.values(gradeByCWS[a] || {}).reduce((sum, val) => sum + val, 0);
            const totalB = Object.values(gradeByCWS[b] || {}).reduce((sum, val) => sum + val, 0);
        ...
        const distributionData = sortedCWS.map(cws => {
            ...
            cwsData.total = mainGrades.reduce((sum, grade) => sum + (gradeByCWS[cws][grade] || 0), 0);
    ...
    const prepareCwsGradeContribution = (gradeByCWS) => {
        ...
        allGrades.forEach(grade => {
            ...
            const totalForGrade = Object.values(gradeByCWS).reduce((sum, cwsGrades) => {
            ...
            Object.entries(gradeByCWS).forEach(([cwsName, cwsGrades]) => {
                ...
                const percentage = totalForGrade > 0 ? (amount / totalForGrade) * 100 : 0;
        ...
        completedBaggingOff.forEach(item => {
            ...
            if (item.outputKgs) {
                Object.entries(item.outputKgs).forEach(([grade, kg]) => {
                    ...
                    gradeByCWS[cwsName][grade] = (gradeByCWS[cwsName][grade] || 0) + kgValue;
                });
            }
        });

        // Calculate total cherry processed from processing data
        const totalCherry = relevantProcessing.reduce((sum, item) => sum + (parseFloat(item.totalKgs) || 0), 0);

        // Calculate total parchment (sum of all grades)
        const totalParchment = Object.values(gradeStock).reduce((sum, kg) => sum + (parseFloat(kg) || 0), 0);

        // Calculate processing yield
        const processingYield = totalCherry > 0 ? (totalParchment / totalCherry) * 100 : 0;
    ...
    const getFilteredData = () => {
        if (selectedCWS === 'All') {
            ...
            const cwsTotalParchment = Object.values(cwsGrades).reduce((sum, kg) => sum + parseFloat(kg), 0);
    ...
    return (
        <div className="container-fluid p-4" style={{ backgroundColor: '#f8fafa' }}>
            <div className="d-flex justify-content-between align-items-center mb-4">
                <h4 className="mb-0 text-sucafina" style={{ color: theme.primary }}>Stock Dashboard</h4>

                <div className="d-flex align-items-center">
                    <label htmlFor="cwsSelect" className="me-2 text-muted">CWS:</label>
                    <select
                        id="cwsSelect"
                        value={selectedCWS}
                    ...
                    >
                        {cwsList.map(cws => (
                            <option key={cws} value={cws}>{cws}</option>
                        ))}
                    </select>
                </div>
            </div>
            ...
            <div className="row g-4 mb-4">
                <div className="col-12 col-md-6">
                    <DashboardCard
                        title="Total Cherry Processed (kg)"
                ...
                <div className="col-12 col-md-6">
                    <DashboardCard
                        title={selectedCWS === 'All' ? 'Total Parchment (kg)' : `${selectedCWS} Parchment (kg)`}
                                            ...
                                            <tr key={index}>
                                                ...
                                                {allGrades.map(grade => (
                                                    <td key={grade} className="text-end">
                                                        {station[grade] ? station[grade].toLocaleString() : '0'}
                                                    </td>
                                                ))}
                                                <td className="text-end fw-bold">{station.total.toLocaleString()}</td>
                                            </tr>
                                        ))}
                                        <tr className="table-light fw-bold">
                                            <td>Total</td>
                                            {allGrades.map(grade => {
                                                const gradeTotal = detailedTableData.reduce((sum, station) => 
                                                    sum + (station[grade] || 0), 0);
                                                return (
                                                    <td key={grade} className="text-end">
                                            ...
                                            <td className="text-end">
                                                {detailedTableData.reduce((sum, station) => 
                                                    sum + station.total, 0).toLocaleString()}
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
